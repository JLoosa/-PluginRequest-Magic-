package me.jrl1004.plugins.magic.particles;
...
import me.jrl1004.plugins.magic.particles.ReflectionUtils.PackageType;
...
public enum ParticleEffect {
	...
	private static boolean isLongDistance(Location location, List<Player> players) {
		for (Player player : players) {
			if (player.getLocation().distanceSquared(location) > LONG_DISTANCE_SQUARED) { return true; }
		}
		return false;
	}
	...
	public void display(float offsetX, float offsetY, float offsetZ, float speed, int amount, Location center, double range) throws ParticleVersionException, ParticleDataException, IllegalArgumentException {
		...
		if (requiresWater && !isWater(center)) { throw new IllegalArgumentException("There is no water at the center location"); }
	...
	public static final class ParticlePacket {
		 ...
		 *        range for particles is usually 16, but it can differ for some
		 *        types)
